...
-- 2025-08-014,HNguyen,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_HNguyen')
	 Begin 
	  Alter Database [Assignment07DB_HNguyen] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_HNguyen;
	 End
	Create Database Assignment07DB_HNguyen;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_HNguyen;
...
SELECT 
    ProductName,
    InventoryDateFormatted AS InventoryDate,
    InventoryCount
FROM vProductInventories
ORDER BY ProductName, RawInventoryDate;
...
SELECT 
    c.CategoryName,
    i.InventoryDate AS RawInventoryDate,  -- keep for ordering
    FORMAT(i.InventoryDate, 'MMMM, yyyy') AS InventoryDateFormatted,
...
ORDER BY CategoryName, RawInventoryDate;
...
SELECT 
    p.ProductName,
    i.InventoryDate AS RawInventoryDate,        -- keep original date
    FORMAT(i.InventoryDate, 'MMMM, yyyy') AS InventoryDate,  -- formatted for display
    i.Count AS InventoryCount,
    ISNULL(
        LAG(i.Count) OVER (
...
SELECT 
    ProductName,
    FORMAT(RawInventoryDate, 'MMMM, yyyy') AS InventoryDate,  -- formatted string for display
    RawInventoryDate,                                         -- keep original date for sorting
    InventoryCount,
    PreviousMonthCount,
    CASE 
